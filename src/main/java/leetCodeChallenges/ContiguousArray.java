 ...
 * 
 *         Example 2: Input: [0,1,0] Output: 2 Explanation: [0, 1] (or [1, 0])
...
public class ContiguousArray {
	...
	/**
	 * Approach #1 Brute Force [Time Limit Exceeded] Algorithm
	 * 
	 * The brute force approach is really simple. We consider every possible
	 * subarray within the given array and count the number of zeros and ones in
	 * each subarray. Then, we find out the maximum size subarray with equal no. of
	 * zeros and ones out of them.
	 */

	public int findMaxLengthApproachOne(int[] nums) {
		int maxlen = 0;
...
//
//Space complexity : O(1)O(1). Only two variables zeroes and ones are required.
//

	/**
	 * Approach #2 Using Extra Array [Accepted]
	 * 
	 * Algorithm In this approach, we make use of a countcount variable, which is
	 * used to store the relative number of ones and zeros encountered so far while
	 ...
	 * encountered. We start traversing the array from the beginning. If at any
	 * moment, the countcount becomes zero, it implies that we've encountered equal
	 * number of zeros and ones from the beginning till the current index of the
	 * array(ii). Not only this, another point to be noted is that if we encounter
	 * the same countcount twice while traversing the array, it means that the
	 * number of zeros and ones are equal between the indices corresponding to the
	 * equal countcount values. The following figure illustrates the observation for
	 * the sequence [0 0 1 0 0 0 1 1]:
	 * 
	 ...
	 * countcount values.
	 */
	public int findMaxLengthApproachTwo(int[] nums) {
...
//
	...
	/**
	 * Approach #3 Using HashMap [Accepted] Algorithm
	 * 
	 * This approach relies on the same premise as the previous approach. But, we
	 * need not use an array of size \text{2n+1}2n+1, since it isn't necessary that
	 * we'll encounter all the countcount values possible. Thus, we make use of a
	 * HashMap mapmap to store the entries in the form of (index,
	 * count)(index,count). We make an entry for a countcount in the mapmap whenever
	 * the countcount is encountered first, and later on use the correspoding index
	 * to find the length of the largest subarray with equal no. of zeros and ones
	 * when the same countcount is encountered again.
	 * 
	 * The following animation depicts the process:
	 */
public int findMaxLengthApproachThree(int[] nums) {
