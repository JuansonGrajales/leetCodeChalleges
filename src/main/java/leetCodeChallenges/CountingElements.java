 ...
 *         2 and 3 are in arr. Example 2:
 * 
 *         Input: arr = [1,1,3,3,5,5,7,7] Output: 0 Explanation: No numbers are
 *         counted, cause there's no 2, 4, 6, or 8 in arr. Example 3:
 * 
 *         Input: arr = [1,3,2,3,5,0] Output: 3 Explanation: 0, 1 and 2 are
 *         counted cause 1, 2 and 3 are in arr. Example 4:
 * 
...
public class CountingElements {
	...
	public static int countElements(int[] arr) {
		// use a hashset as a reference for all the sum values
		...
		for (int var : arr) {
			int elementPlusOne = var + 1;
			if (set.contains(elementPlusOne)) {
				result++;
			}
		}
