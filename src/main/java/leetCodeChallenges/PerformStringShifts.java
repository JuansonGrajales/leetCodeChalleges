 ...
 *         Explanation:[0,1] means shift to left by 1. "abc" -> "bca" [1,2]
 *         means shift to right by 2. "bca" -> "cab" Example 2:
 * 
...
public class PerformStringShifts {
	...
	public static String stringShift(String s, int[][] shift) {
		...
		for (int i = 0; i < shift.length; i++) {
			...
			if (shift[i][0] == 0) {
				leftWeight += shift[i][1];
			} else {
				rightWeight += shift[i][1];
			}
		}

		int difference = Math.abs(rightWeight - leftWeight);
		while(difference > s.length()) {
			difference -= s.length();
		}

		if (leftWeight == rightWeight || difference == s.length()) {
			return s;
		} else if (leftWeight > rightWeight) {
			s = s.substring(difference) + s.substring(0, difference);
		} else {
			s = s.substring(s.length() - difference) + s.substring(0, s.length() -difference);
		}
		return s;
	}
